...
@onready var Joystick:Control = find_interaction_dialog('VirtualJoystick', 'analogico')

@onready var flowerHud: Sprite2D = find_interaction_dialog('FlowerHud', 'Hud')
var hasFlowerHud: bool = false

var isDialogActive: bool = false
var onCollisionField: bool = false
var isFinalOption: bool = false

func find_node_by_group(group_name: String, node_name: String) -> Node:
    var nodes = get_tree().get_nodes_in_group("analogico")
    
    for node in nodes:
        if node.name == node_name:
            return node
    
    print("Erro: Nenhum nó chamado", node_name, "no grupo", group_name)
    return null
